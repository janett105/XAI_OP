...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    """# 이진 분류를 위한 ViT의 model head (FC layer) 변경
    num_ftrs = model.head.in_features # head 입력 특성 수 가져오기
    model.head = torch.nn.Linear(num_ftrs, 2, bias=True) # 출력 크기를 2로 설정하는 Linear layer로 변경"""
    
    model.to(device)  # 모델을 적절한 디바이스로 이동
    model.train(True) # 변경된 모델 구조를 사용하도록 설정
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            # print('outputs', outputs.shape, 'targets_one_hot', targets_one_hot.shape)
...
def computeAUROC(targets, outputs, num_classes):
    outAUROC = []  # 초기화
    actual_num_classes = outputs.shape[1]
    
    for i in range(num_classes):
        try:
            outAUROC.append(roc_auc_score(targets[:, i], outputs[:, i]))
        except ValueError:
            # AUC를 계산할 수 없는 경우 NaN으로 처리
    ...
    if num_classes==2:
        try:
            outAUROC = roc_auc_score(targets, outputs)
        except:
            outAUROC=0.
