...
from engine_finetune import computeAUROC
...
class Solver(object):
    ...
    def load_model(self):
        ...
        if self.model=='densenet121':
            ...
            self.model.classifier = torch.nn.Linear(num_ftrs, 1, bias=True).to(self.device)
    ...
    def train(self):
        ...
        train_correct = 0
        total=0
        for _, (data_batch, labels) in enumerate(self.data_loader_train):
        ...
        return train_loss / len(self.data_loader_train), train_correct / total
    ...
    def evaluate(self, mode):
        ...
        if mode=='val':valtest_loader = self.data_loader_val
        else:valtest_loader = self.data_loader_test
        ...
        auc = computeAUROC(targets, outputs, num_classes=2)
                
        # len(valtest_loader)로 나눔
        return valtest_loss / len(valtest_loader), valtest_correct / total, auc
    
    def save_model_state(self):
        directory = f"results/imagenet_mae/densenet121"
        if not os.path.exists(directory):
            os.makedirs(directory)  # 디렉토리가 없다면 생성
        path = f"{directory}/bestmodel.pth"
        torch.save(self.model.state_dict(), path)
        print("Checkpoint saved to {}".format(path))

    def test(self):
        #self.model은 이미 모델의 인스턴스니까 () 필요 없음        
        self.model.load_state_dict(torch.load(f'results/imagenet_mae/densenet121/bestmodel.pth'))
        test_loss, test_acc = self.evaluate(mode='test')  # 모델을 평가
        return test_loss, test_acc

    def run(self):
        print(f'<finetuning for {self.epochs} epochs>')
        ...
        for epoch in range(1, self.epochs+1):
            ...
            val_loss, val_acc, val_auc = self.evaluate(mode='val')
        ...
        print(f"===> BEST VAL_AUC: {best_val_auc:.3f}")
        test_loss, test_acc, test_auc = self.test()
        ...
        #             best_val_auc = val_auc
        #         self.scheduler.step() # PyTorch의 더 깨끗하고 직관적인 API 사용을 장려하기 위해 torch.optim.lr_scheduler의 사용법이 변경되어 epoch 삭제
        #     print(f"===> {fold}Fold BEST VAL_ACC: {best_val_acc:.3f}")
        #     print(f"===> {fold}Fold BEST VAL_AUC: {best_val_auc:.3f}")
        #     test_loss, test_acc, test_auc = self.test(fold)
        #     print(f"===> {fold}Fold TEST_ACC: {test_acc:.3f}")
        #     print(f"===> {fold}Fold TEST_AUC: {test_auc:.3f}")
        #     self.metrics_df.loc[fold, 'acc'] = test_acc
        #     self.metrics_df.loc[fold, 'auc'] = test_auc
        # print(f"10 Fold TEST:\n{self.metrics_df}")
        # print(f'Final test Acc: {self.metrics_df["acc"].mean():.3f} ({self.metrics_df["acc"].std():.3f})')
        # print(f'Final test Auc: {self.metrics_df["auc"].mean():.3f} ({self.metrics_df["auc"].std():.3f})')
